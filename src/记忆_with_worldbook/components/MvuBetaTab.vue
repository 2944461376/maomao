...
async function handleAIGenerateStructure() {
  ...
  try {
    const systemPrompt = `你是 MVU Beta 专家。生成 [InitVar] JSON。
...
async function handleAIGeneratePrompt() {
  ...
  try {
    const systemPrompt = `你是 MVU Beta COT 提示词专家。生成完整的 6 步思维链提示词。

## 要求：
1. 包含 {{get_message_variable::stat_data}}
2. 使用 _.insert 而非 _.assign
3. 所有路径使用 [0] 后缀
4. 6 步 COT 流程完整
...
async function modifyStructureWithAI(modifyInstruction: string) {
  ...
  try {
    ...
    const systemPrompt = `你是 MVU Beta 专家。根据原始需求和修改建议，重新生成 [InitVar] JSON。
...
async function modifyPromptWithAI(modifyInstruction: string) {
  ...
  try {
    ...
    const systemPrompt = `你是 MVU Beta COT 提示词专家。根据原始场景描述和修改建议，重新生成 6 步思维链提示词。
